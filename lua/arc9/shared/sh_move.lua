...
function ARC9.StartCommand(ply, cmd)
    ...
    if CLIENT and IsValid(wep) and wep.ARC9 then
            -- and (wep:GetBuff("AimAssist", true) or (GetConVar("arc9_aimassist"):GetBool() and ply:GetInfoNum("arc9_aimassist_cl", 0) == 1))  then
        -- local cone = wep:GetBuff("AimAssist", true) and wep:GetBuff("AimAssist_Cone") or GetConVar("arc9_aimassist_cone"):GetFloat()
        -- local dist = wep:GetBuff("AimAssist", true) and wep:GetBuff("AimAssist_Distance") or GetConVar("arc9_aimassist_distance"):GetFloat()
        -- local inte = wep:GetBuff("AimAssist", true) and wep:GetBuff("AimAssist_Intensity") or GetConVar("arc9_aimassist_intensity"):GetFloat()
        -- local head = wep:GetBuff("AimAssist", true) and wep:GetBuff("AimAssist_Head") or GetConVar("arc9_aimassist_head"):GetBool()
		
            -- and (GetConVar("arc9_aimassist"):GetBool() and ply:GetInfoNum("arc9_aimassist_cl", 0) == 1) then
        local cone = GetConVar("arc9_aimassist_cone"):GetFloat()
        local dist = GetConVar("arc9_aimassist_distance"):GetFloat()
        local inte = GetConVar("arc9_aimassist_intensity"):GetFloat()
        local head = GetConVar("arc9_aimassist_head"):GetBool()
        ...
        -- Aim towards target
        tgt = ply.ARC9_AATarget
        if !wep:GetCustomize() and IsValid(tgt) then
