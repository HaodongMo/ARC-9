...
local move_dis = GetConVar("arc9_movedispersion")
...
do
    ...
    function SWEP:GetProcessedValue(val, cachedelay, base, cmd)
		...
		local adsmult = move_dis:GetBool() and self:GetInSights()
        ...
        if not ownerIsNPC and entityIsValid(owner) then
            ...
            if not adsmult and not ownerOnGround or entityGetMoveType(owner) == MOVETYPE_NOCLIP then
        ...
        if not adsmult and val ~= "RecoilModifierCap" and not hasNoAffectors[val .. "Recoil"] then
        ...
        if not adsmult and not hasNoAffectors[val .. "Move"] and IsValid(owner) then
