...
function SWEP:GetViewModelPosition(pos, ang)
    ...
    if curvedcustomizedelta > 0 then
        if !self.CustomizeNoRotate then
            ...
            local rap_pos, rap_ang = self:RotateAroundPoint2(pos, ang, self:GetProcessedValue("CustomizeRotateAnchor") or Vector(21.5, -4.27, -5.23), vector_origin, rotateAroundAngle)
            pos:Set(rap_pos)
            ang:Set(rap_ang)
        end
    end
