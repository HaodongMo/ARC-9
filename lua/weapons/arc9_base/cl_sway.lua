...
local hip_sway = GetConVar("arc9_vm_hipsway")
local ads_sway = GetConVar("arc9_vm_adssway")

local function FesiugBob(self, pos, ang, insight)
    ...
    pos:Add( ang:Right()     *   stammertime_pos.x * elistam * sharedmult * insight )
    pos:Add( ang:Forward()   *   stammertime_pos.y * elistam * sharedmult * insight )
    pos:Add( ang:Up()        *   stammertime_pos.z * elistam * sharedmult * insight )

    ang:RotateAroundAxis( ang:Forward(),        affset.x * tv * sharedmult * insight )
    ang:RotateAroundAxis( ang:Right(),          affset.y * tv * sharedmult * insight )
    ang:RotateAroundAxis( ang:Up(),             affset.z * tv * sharedmult * insight )

    ang:RotateAroundAxis( ang:Forward(),        stammertime_ang.x * elistam * sharedmult * insight )
    ang:RotateAroundAxis( ang:Right(),          stammertime_ang.y * elistam * sharedmult * insight )
    ang:RotateAroundAxis( ang:Up(),             stammertime_ang.z * elistam * sharedmult * insight )
    ang:RotateAroundAxis( ang:Up(),             (owner:KeyDown(IN_MOVELEFT) and 2 or owner:KeyDown(IN_MOVERIGHT) and -2 or 0) * tv * insight )

    ang:RotateAroundAxis( ang:Forward(),          math.sin( ct * p * 1 ) * airtime*-5 * mulp * 2 * insight )
    ang:RotateAroundAxis( ang:Right(),          ( math.sin( ct * p * 1 ) * airtime*3 * mulp ) + ( (3/2) * airtime * mulp ) * insight ) 
    ang:RotateAroundAxis( ang:Up(),          math.sin( ct * p * 2 ) * airtime*2 * mulp * insight )

    return pos, ang
end

local function ArcticBob(self, pos, ang, insight)
    local step = 10
    local mag = 1
    ...
    step = 10
    ang:RotateAroundAxis(ang:Forward(), math.sin(self.BobCT * step * 0.5) * ((math.sin(self.BobCT * 6.151) * 0.2) + 1) * 4.5 * d * sharedmult * insight)
    ang:RotateAroundAxis(ang:Right(), math.sin(self.BobCT * step * 0.12) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 2.11 * d * sharedmult * insight)
    pos = pos - (ang:Up() * math.sin(self.BobCT * step) * 0.1 * ((math.sin(self.BobCT * 3.515) * 0.2) + 1) * mag * sharedmult * insight)
    pos = pos + (ang:Forward() * math.sin(self.BobCT * step * 0.3) * 0.11 * ((math.sin(self.BobCT * 2) * ts * 1.25) + 1) * ((math.sin(self.BobCT * 1.615) * 0.2) + 1) * mag * sharedmult * insight)
    pos = pos + (ang:Right() * (math.sin(self.BobCT * step * 0.15) + (math.cos(self.BobCT * step * 0.3332))) * 0.16 * mag * sharedmult * insight)
...
local function ArcticBreadBob(self, pos, ang, insight)
    local step = 10
    local mag = 1
    local ts = 0 -- self:GetTraversalSprintAmount()
    -- ts = 1
    ...
    if owner:IsSprinting() then 
        pos = pos - (ang:Up() * math.sin(self.BobCT * step) * 0.45 * ((math.sin(self.BobCT * 3.515) * 0.2) + 1) * mag * sharedmult * insight)
        pos = pos + (ang:Forward() * math.sin(self.BobCT * step * 0.3) * 0.11 * ((math.sin(self.BobCT * 2) * ts * 1.25) + 1) * ((math.sin(self.BobCT * 0.615) * 0.2) + 2) * mag * sharedmult * insight)
        pos = pos + (ang:Right() * (math.sin(self.BobCT * step * 0.5) + (math.cos(self.BobCT * step * 0.5))) * 0.55 * mag * sharedmult * insight)
        ang:RotateAroundAxis(ang:Forward(), math.sin(self.BobCT * step * 0.5) * ((math.sin(self.BobCT * 6.151) * 0.2) + 1) * 9 * d * sharedmult + smoothsidemove * 1.5 * insight)
        ang:RotateAroundAxis(ang:Right(), math.sin(self.BobCT * step * 0.12) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 1 * d * sharedmult)
        ang:RotateAroundAxis(ang:Up(), math.sin(self.BobCT * step * 0.5) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 6 * d * sharedmult * insight)
        ang:RotateAroundAxis(ang:Right(), smoothjumpmove2 * 5)
    else
        pos = pos - (ang:Up() * math.sin(self.BobCT * step) * 0.1 * ((math.sin(self.BobCT * 3.515) * 0.2) + 2) * mag * crouchmult * sharedmult * insight) - (ang:Up() * smoothsidemove * -0.05) - (ang:Up() * smoothjumpmove2 * 0.2)
        pos = pos + (ang:Forward() * math.sin(self.BobCT * step * 0.3) * 0.11 * ((math.sin(self.BobCT * 2) * ts * 1.25) + 1) * ((math.sin(self.BobCT * 0.615) * 0.2) + 1) * mag * sharedmult * insight)
        pos = pos + (ang:Right() * (math.sin(self.BobCT * step * 0.5) + (math.cos(self.BobCT * step * 0.5))) * 0.55 * mag * sharedmult * insight)
        ...
        ang:RotateAroundAxis(ang:Right(), math.sin(self.BobCT * step * 0.12) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 0.1 * d * sharedmult * insight)
        ang:RotateAroundAxis(ang:Right(), smoothjumpmove2 * 5)
    end
...
local function ArcticBreadDarsuBob(self, pos, ang, insight)
    local step = 10
    local mag = 1
    local ts = 0 -- self:GetTraversalSprintAmount()
    -- ts = 1
    ...
    if owner:IsSprinting() then 
        pos = pos - (ang:Up() * math.sin(self.BobCT * step) * 0.45 * ((math.sin(self.BobCT * 3.515) * 0.2) + 1) * mag * sharedmult * insight)
        pos = pos + (ang:Forward() * math.sin(self.BobCT * step * 0.3) * 0.13 * ((math.sin(self.BobCT * 2) * ts * 1.25) + 2) * ((math.sin(self.BobCT * 0.615) * 0.2) + 2) * mag * sharedmult * insight)
        pos = pos + (ang:Right() * (math.sin(self.BobCT * step * 0.5) + (math.cos(self.BobCT * step * 0.5))) * 0.55 * mag * sharedmult * insight)
        ...
        ang:RotateAroundAxis(ang:Right(), math.sin(self.BobCT * step * 0.12) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 1 * d * sharedmult * insight)
        ang:RotateAroundAxis(ang:Up(), math.sin(self.BobCT * step * 0.5) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 6 * d * sharedmult * insight)
        ang:RotateAroundAxis(ang:Right(), smoothjumpmove2 * 5)
    else
        pos = pos - (ang:Up() * math.sin(self.BobCT * step) * 0.1 * ((math.sin(self.BobCT * 3.515) * 0.2) + 1.5) * mag * crouchmult * sharedmult * insight) - (ang:Up() * smoothsidemove * -0.05) - (ang:Up() * smoothjumpmove2 * 0.2)
        pos = pos + (ang:Forward() * (math.sin(self.BobCT * step * 0.3) * 0.11 * ((math.sin(self.BobCT * 2) * ts * 1.25) + 1) * ((math.sin(self.BobCT * 0.615) * 0.2) + 1) * mag * sharedmult)  * insight)
        pos = pos + (ang:Right() * ((math.sin(self.BobCT * step * 0.5) + (math.cos(self.BobCT * step * 0.5))) * 0.2 * mag * sharedmult) * insight)
        ang:RotateAroundAxis(ang:Forward(), math.sin(self.BobCT * step * 0.5) * ((math.sin(self.BobCT * 6.151) * 0.2) + 1) * 5 * d * sharedmult + smoothsidemove)
        ang:RotateAroundAxis(ang:Right(), (math.sin(self.BobCT * step * 0.12) * ((math.sin(self.BobCT * 1.521) * 0.2) + 1) * 0.1 * d * sharedmult) * insight)
        ang:RotateAroundAxis(ang:Right(), smoothjumpmove2 * 5)
    end
...
local function DarsuBob(self, pos, ang, insight)
    ...
    pos:Sub(ang:Right() *          math.sin(speedmult * self.BobCT * 5 * xms)  * d2 * 0.5 * Lerp(sprintamount, 1, 0.05) * xm * sharedmult * insight)                                   -- X 
    pos:Sub(ang:Up() *             math.cos(speedmult * self.BobCT * 7 * yms)    * d * 0.05 * (crouchmult*crouchmult*crouchmult) * Lerp(sprintamount, 1, 0.3) * ym * sharedmult * insight)                     -- Y
    pos:Sub(ang:Forward() *        math.sin(speedmult * self.BobCT * 4 * zms)  * d2 * 0.75 * crouchmult * zm * sharedmult * insight)                   -- Z

    ang:RotateAroundAxis(ang:Right(),   math.sin(speedmultang * self.BobCT * 5.5 * pms + 0.3)    * d3 * 2.25 * pm * sharedmult + smoothjumpmove * insight)                  -- P
    ang:RotateAroundAxis(ang:Up(),      math.cos(speedmultang * self.BobCT * 3.3 * yyms)  * d3 * 1 * Lerp(sprintamount, 1, 0.1) * yym * sharedmult * insight)                                   -- Y
    ang:RotateAroundAxis(ang:Forward(), math.sin(speedmultang * self.BobCT * 6 * rms)    * d3 * 4.5 * crouchmult * rm * sharedmult + smoothsidemove * insight)   -- R

    return pos, ang
end

local function ArcticClassicBob(self, pos, ang, insight)
    if self:GetCustomize() then return pos, ang end
	
	local owner = self:GetOwner()
    local ft = FrameTime()
	
    local speedmult = owner:IsSprinting() and self.BobSprintMult or self.BobWalkMult
    local v = math.Clamp(owner:GetVelocity():Length(), 0, 350)
	self.ViewModelBobVelocity = math.Approach(self.ViewModelBobVelocity, v, ft * 10000)
    ...
    if owner:OnGround() and owner:GetMoveType() != MOVETYPE_NOCLIP then
        self.ViewModelNotOnGround = math.Approach(self.ViewModelNotOnGround, 0, ft / 0.1)
		ang:RotateAroundAxis(ang:Up(), math.sin(self.BobCT * step * 0.5) * -0.5 * d * speedmult * insight)
		ang:RotateAroundAxis(ang:Forward(), math.sin(self.BobCT * step * 0.5) * -0.5 * speedmult * mag * insight)
		pos = pos + (ang:Up() * math.sin(self.BobCT * step * 1) * v * (0.0007) * speedmult * mag * insight)
		pos = pos + (ang:Right() * math.sin(self.BobCT * step * 0.5) * v * (0.001) * speedmult * mag * insight)
    else
        self.ViewModelNotOnGround = math.Approach(self.ViewModelNotOnGround, 1, ft / 0.1)
    end
	
    if IsFirstTimePredicted() or game.SinglePlayer() then
		local steprate = Lerp(self.ViewModelNotOnGround, Lerp(d, 1, 2.5), 0.25)
        self.BobCT = self.BobCT + (ft * steprate)
    end
...
function SWEP:GetViewModelBob(pos, ang)
	...
	local hipmult, adsmult = hip_sway:GetFloat(), ads_sway:GetFloat()
	local insight = Lerp(self:GetSightAmount(),1*hipmult,1*adsmult)

    if bobb == 1 then
        return FesiugBob(self, pos, ang, insight)
    elseif bobb == 2 then
        return ArcticBob(self, pos, ang, insight)
    elseif bobb == 3 then
        return DarsuBob(self, pos, ang, insight)
    elseif bobb == 4 then
        return ArcticBreadBob(self, pos, ang, insight)
	elseif bobb == 5 then
		return ArcticClassicBob(self, pos, ang, insight)
    elseif bobb == 0 then
        return ArcticBreadDarsuBob(self, pos, ang, insight)
    else
		local hipmult, adsmult = hip_sway:GetInt(), ads_sway:GetInt()
        self.SwayScale = Lerp(self:GetSightDelta(), 1, 0.01)
        self.BobScale = Lerp(self:GetSightDelta(), 1*hipmult, 0.01*adsmult)
...
local midair_sway = GetConVar("arc9_vm_airsway")

function SWEP:GetMidAirBob(pos, ang)
    if self:GetCustomize() then return pos, ang end
	local airmult = midair_sway:GetFloat()
    ...
    d = d * Lerp(self:GetSightAmount(), 1.5, 0.1)
    -- ang:RotateAroundAxis(ang:Right(), -v * d * 8 * math.sin(CurTime() * 0.15))
    pos = pos + ang:Up() * -v * d * airmult
