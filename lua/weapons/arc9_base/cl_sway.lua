...
local function FesiugBob(self, pos, ang)
	...
	local hipmult, adsmult = hip_sway:GetString(), ads_sway:GetString()
    ...
    local tv = v / 400
    tv = tv * 1.1
    local mulp = Lerp(self:GetSightDelta(), 1*hipmult, 0.15*adsmult)
...
local function ArcticBob(self, pos, ang)
    ...
    d = d * Lerp(self:GetSightAmount(), 1*hipmult, 0.5*adsmult) * Lerp(ts, 1, 1.5)
    mag = d * 2
...
local function ArcticBreadBob(self, pos, ang)
    ...
    d = d * Lerp(sightamount, 1*hipmult,0.03*adsmult) * Lerp(ts, 1, 1.5)
    mag = d * 2
    mag = mag * Lerp(ts, 1, 2)
    step = 10
...
local function DarsuBob(self, pos, ang)
    ...
    d = d * Lerp(sightamount, 1.4*hipmult, 0.8*adsmult) -- If we in sights make less moves
...
local function ArcticClassicBob(self, pos, ang)
	...
	local d = math.Clamp(self.ViewModelBobVelocity / 350, 0, 1)
	d = d * Lerp(self:GetSightAmount(), 1*hipmult, 0.1*adsmult)
    local mag = (d * 2)
	local step = 10

    if owner:OnGround() and owner:GetMoveType() != MOVETYPE_NOCLIP then
        self.ViewModelNotOnGround = math.Approach(self.ViewModelNotOnGround, 0, ft / 0.1)
		ang:RotateAroundAxis(ang:Up(), math.sin(self.BobCT * step * 0.5) * -0.5 * d * speedmult)
		ang:RotateAroundAxis(ang:Forward(), math.sin(self.BobCT * step * 0.5) * -0.5 * speedmult * mag)
		pos = pos + (ang:Up() * math.sin(self.BobCT * step * 1) * v * (0.0007) * speedmult * mag)
		pos = pos + (ang:Right() * math.sin(self.BobCT * step * 0.5) * v * (0.001) * speedmult * mag)
    else
        self.ViewModelNotOnGround = math.Approach(self.ViewModelNotOnGround, 1, ft / 0.1)
    end
...
function SWEP:GetViewModelBob(pos, ang)
    ...
    elseif bobb == 0 then
        ...
        self.SwayScale = Lerp(self:GetSightDelta(), 1*hipmult, 0.01*adsmult)
...
function SWEP:GetMidAirBob(pos, ang)
    if self:GetCustomize() then return pos, ang end
	local airmult = midair_sway:GetString()
